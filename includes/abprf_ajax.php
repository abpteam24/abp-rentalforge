			...
			public function load_property() {
				if ( check_ajax_referer( 'abprf_ajax_nonce', 'nonce' ) ) {
					...
					if ( $rent_rule == 'hourly' && ! empty( $rent_start_date ) ) {
						...
						$dif_infos                            = array_key_exists( 'info', $differ ) ? $differ['info'] : [];
						$hour_dif                             = array_key_exists( 'hour', $dif_infos ) ? $dif_infos['hour'] : 0;
						$min_dif                              = array_key_exists( 'min', $dif_infos ) ? $dif_infos['min'] : 0;
						$hour_dif                             = $min_dif > 0 ? $hour_dif + 1 : $hour_dif;
						$abprf_infos['date_info']['dif']      = $hour_dif;
					...
					if ( ! empty( $properties ) && is_array( $properties ) && sizeof( $properties ) > 0 && is_array( $date_infos ) && sizeof($date_infos) > 0 ) {
						foreach ( $properties as $property ) {
							$post_id   = array_key_exists( 'post_id', $property ) ? $property['post_id'] : '';
							$post_data = array_key_exists( $post_id, $date_infos ) ? $date_infos[ $post_id ] : $date_infos['global'];
