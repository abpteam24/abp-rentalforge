        ...
        <div class=" abptm_list abprf_pagination_area">
            <div class="_f_wrap_mar_t_xs">
				<?php foreach ($all_transport_ids as $transport_id => $title) {
                    ...
                    <a class="_abprf_margin_xxs_border_b pagination_item  <?php echo esc_attr($show_post >= $post_count ? '' : '_d_none'); ?>" href="<?php echo esc_url(get_the_permalink($transport_id) . '?_bp= ' . $from . '&_dp=' . $to); ?>">
						<?php
							echo esc_html($title);
